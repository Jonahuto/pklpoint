...
// Cek apakah dosen sedang ingin melihat laporan dari mahasiswa tertentu
$target_user_id = isset($_GET['user_id']) ? (int)$_GET['user_id'] : null;

// Ambil semua user_id mahasiswa bimbingan dosen ini
$result_mahasiswa = $conn->query("SELECT user_id FROM pkl_data WHERE pembimbing = $user_id");

$mahasiswa_ids = [];
while ($row = $result_mahasiswa->fetch_assoc()) {
    $mahasiswa_ids[] = $row['user_id'];
}

if (!empty($mahasiswa_ids)) {
    if ($target_user_id && in_array($target_user_id, $mahasiswa_ids)) {
        // Hanya ambil laporan dari mahasiswa yang dipilih
        $laporan_query = $conn->query("
            SELECT l.*, u.nama_lengkap 
            FROM laporan_akhir l 
            JOIN users u ON l.id_user = u.user_id 
            WHERE l.id_user = $target_user_id
            ORDER BY l.tanggal DESC
        ");
    } else {
        // Ambil semua laporan dari mahasiswa bimbingan
        $id_list = implode(',', $mahasiswa_ids);
        $laporan_query = $conn->query("
            SELECT l.*, u.nama_lengkap 
            FROM laporan_akhir l 
            JOIN users u ON l.id_user = u.user_id 
            WHERE l.id_user IN ($id_list)
            ORDER BY l.tanggal DESC
        ");
    }
} else {
...
if (isset($_POST['submit_komentar']) && isset($_POST['id_kegiatan'])) {
                    $komentar = $conn->real_escape_string($_POST['komentar_' . $_POST['id_kegiatan']]);
                    $laporan_id = (int)$_POST['id_kegiatan'];
                    $tanggal_komentar = date('Y-m-d H:i:s');

                    $update = $conn->query("UPDATE laporan_akhir SET komentar = '$komentar', tanggal_komentar = '$tanggal_komentar' WHERE id_kegiatan = $laporan_id");

                    if ($update) {
                        echo "<script>alert('Komentar berhasil dikirim.'); window.location.href = 'lihat_laporan_akhir.php?user_id=$target_user_id';</script>";
                    } else {
...
<head>
    ...
    <link rel="stylesheet" href="../css/dosen/lihat_laporan_akhir.css">
</head>
    ...
    <div class="container">
        <aside class="sidebar">
            ...
            <nav>
                ...
                <a href="laporanakhirmhs.php"><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000" alttext="Logo Laporan Akhir">
        ...
        <main class="main-content">
            ...
            <section class="dashboard">
                ...
                <hr class="separator">
                <h2>Detail Laporan Akhir</h2>

                <?php if ($laporan_query && $laporan_query->num_rows > 0): ?>
                    <?php while ($laporan = $laporan_query->fetch_assoc()): ?>
                        <table class="laporan-detail-table">
                            ...
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($laporan['judul_laporan_akhir']); ?></td>
                            </tr>
                            <tr>
                                <td><strong>File Laporan</strong></td>
                                <td><a href="../mhs/uploads/<?php echo htmlspecialchars($laporan['file']); ?>" target="_blank">Lihat File</a></td>
                            </tr>
                            <tr>
                                <td><strong>Tanggal Upload</strong></td>
                                <td><?php echo date('d-m-Y H:i:s', strtotime($laporan['tanggal'])); ?></td>
                            </tr>
                            <tr>
                                <td><strong>Komentar / Feedback</strong></td>
                                <td>
                                    <?php if (!empty($laporan['komentar'])): ?>
                                        <?php echo nl2br(htmlspecialchars($laporan['komentar'])); ?>
                                        <p style="font-size: 12px; color: gray;">
                                            Diberikan pada <?php echo date('d-m-Y H:i:s', strtotime($laporan['tanggal_komentar'])); ?>
                                        </p>
                                    <?php else: ?>
                                        <form method="POST">
                                            <textarea name="komentar_<?php echo $laporan['id_kegiatan']; ?>" rows="4" required></textarea>
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <p>Tidak ada laporan akhir dari mahasiswa bimbingan.</p>
                <?php endif; ?>
