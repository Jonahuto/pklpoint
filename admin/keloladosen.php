...
// PHP untuk mengambil data dengan role='dosen'
...
<head>
    ...
    <link rel="stylesheet" href="../css/admin/keloladosen.css">
</head>
    ...
    <div class="container">
        <aside class="sidebar">
            ...
            <nav>
                ...
                <a href="keloladosen.php" class="active"><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000" alttext="Logo Kegiatan PKL">
        ...
        <main class="main-content">
            ...
            <section class="dashboard">
                ...
                <div style="margin-bottom: 10px;">
                    <input type="text" id="searchInput" placeholder="Cari NIP atau Nama Dosen..." style="padding: 8px; width: 300px;">
                </div>
                <table border="1" cellpadding="10" cellspacing="0" style="width:100%; border-collapse:collapse;">
                    ...
                    <tbody>
                        <?php
                        $no = 1;
                        foreach ($dosen_only as $only_dosen) :
                        ?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td><?= !empty($only_dosen['nim']) ? htmlspecialchars($only_dosen['nim']) : '-' ?></td>
                                <td><?= !empty($only_dosen['nama_lengkap']) ? htmlspecialchars($only_dosen['nama_lengkap']) : '-' ?></td>
                                <td><?= !empty($only_dosen['email']) ? htmlspecialchars($only_dosen['email']) : '-' ?></td>
                                <td><?= !empty($only_dosen['no_telp']) ? htmlspecialchars($only_dosen['no_telp']) : '-' ?></td>
                                <td>
                                    <a class="aksi-edit" href="edit_dosen.php?id=<?= $only_dosen['user_id'] ?>">✏️</a>
                                    <a class="aksi-hapus" href="javascript:void(0);" onclick="konfirmasiHapus(<?= $only_dosen['user_id'] ?>)">🗑️</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <div id="popupHapus" class="popup-hapus">
        <div class="popup-hapus-content">
            ...
            <div class="popup-hapus-buttons">
                ...
                <a href="keloladosen.php?hapus=1" id="btnHapusYa" class="btn-hapus-ya">Ya</a>
            </div>
        </div>
    </div>
    <script>
        function konfirmasiHapus(id) {
            ...
            btnHapusYa.href = "hapus_dosen.php?id=" + id;
